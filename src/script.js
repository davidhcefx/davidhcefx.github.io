...
function hl_go() {
    ...
    const libname = hl_get_libname(brush);  // may be undefined
    const dependency = hl_get_dependency(libname);  // may be undefined
    var reminder = '';
    var html = `<html>
    ...
    if (libname !== undefined) {  // not a default library
        ...
        html += `<script src="${CDN}/${js_to_minjs(libname)}">\x3c/script>`;

        if (dependency !== undefined) {
            const deps = `<b>${dependency.join('</b>, <b>')}</b>`;
            reminder += `<br>... as well as its dependencies ${deps}.`;
            dependency.forEach(dep => {
                html += `<script src="${CDN}/${js_to_minjs(dep)}">\x3c/script>`;
            });
        }
        reminder += '</p>';
    ...
    html += `<script>hljs.initHighlightingOnLoad();\x3c/script></head>
        <body><pre><code class="${brush}">${escape_tags(code)}</code></pre>
        \x3c/body>\x3c/html>`;
...
function hl_get_dependency(libname) {
    return {
