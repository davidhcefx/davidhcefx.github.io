...
const CDN_VER = '11.5.1';
const CDN = `https://cdnjs.cloudflare.com/ajax/libs/highlight.js/${CDN_VER}`;

/* Fetch IP and display in #ip-info */
function fetchIP() {  // eslint-disable-line no-unused-vars
  const [time, info] = JSON.parse(localStorage.getItem('ip_info') || '[0, ""]');
  ...
  fetch(`https://api.bigdatacloud.net/data/ip-geolocation?key=${key}`)
    .then((r) => r.json())
    .then((j) => {
      const newInfo = 'You visited this site using '
      ...
      document.querySelector('#ip-info a').innerText = newInfo;
      localStorage.setItem('ip_info', JSON.stringify([Date.now(), newInfo]));
    });
}

/* Resize iframe to the height of its content. */
function resizeHeight(iframe) {
  iframe.height = `${iframe.contentDocument.body.scrollHeight + 26}px`;
}

/* Process input from highlight.js testing tool. */
function doHighlight() {  // eslint-disable-line no-unused-vars
  ...
  const libname = getNonDefaultLibName(lang);
  const dependency = getNonDefaultDependency(libname);
  let reminder = '';
  let html = `<html>
  ...
  if (libname !== undefined) {  // not a default library
    reminder = `* Reminder: "${lang}" is not within the default
      <a href="https://highlightjs.org/download/">34 languages</a>
      bundled by the prebuilt CDN. Be sure to include
      <b>${libname}</b> if you are using highlight.js from CDN`;
    html += `<script src="${CDN}/languages/${jsToMinjs(libname)}">\x3c/script>`;

    if (dependency !== undefined) {
      reminder += `, as well as its dependency <b>${dependency}</b>`;
      html += `<script src="${CDN}/languages/${jsToMinjs(dependency)}">\x3c/script>.`;
    }
  }
  html += `<script>hljs.highlightAll();\x3c/script></head>
    <body><pre><code class="${lang}">${escapeTags(code)}</code></pre>
    \x3c/body>\x3c/html>`;

  document.querySelector('#hl-reminder i').innerHTML = reminder;
  const result = document.getElementById('hl-result');
  result.srcdoc = html;
  result.hidden = false;  // TODO: deprecated for old embeded project
  resizeHeight(result);
}
...
/* Get libname if lang is not a default language, else undefined */
function getNonDefaultLibName(lang) {
...
/* Get non-default dependency(s) or undefined */
function getNonDefaultDependency(libname) {
  return {
